...
class BasicBlock(nn.StatefulLayer):
    ...
    conv2: nn.Conv2d
    bn2: eqx.Module
    downsample: Optional[nn.StatefulLayer]
    ...
    def __call__(self, x: jax.Array, state: nn.State, *, key:Optional[jr.PRNGKey]=None) -> jax.Array:
        identity = x
...
class ResNet(nn.StatefulLayer):
    ...
    def __init__(
        ...
        *,
        ...
        layer1, inplanes = self._make_layer(block, inplanes, 64, layers[0], key=keys[1])
        layer2, inplanes = self._make_layer(
            block,
            inplanes,
            128,
            layers[1],
        ...
        layer3, inplanes = self._make_layer(
            block,
            inplanes,
            256,
            layers[2],
        ...
        layer4, inplanes = self._make_layer(
            block,
            inplanes,
            512,
            layers[3],
    ...
    def _make_layer(
        ...
        *,
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            key_to_use, key = jr.split(key)
            downsample = nn.Sequential(
                [
                    conv1x1(
                        inplanes, planes * block.expansion, stride, key=key_to_use
                    ),
...
def resnet18(bn_momentum=0.0, **kwargs: Any) -> ResNet:
    ...
    return _resnet(BasicBlock, [2, 2, 2, 2], norm_layer=norm_layer, **kwargs)
